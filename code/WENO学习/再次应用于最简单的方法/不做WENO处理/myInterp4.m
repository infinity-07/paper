function yy  = myInterp4(xx, xa, xb, uavem, uave, lambda)
    % 插值函数构造
    % 输入:
    %   xa, xb    区间端点
    %   h         区间长度
...
%     varphi1 = exp(lambda*(x-xc));
    varphi2 = exp(-lambda*(x-xc));
    % varphi3 = (x-xc)*sinh(lambda*(x-xc));
    % varphi4 = (x-xc)*cosh(lambda*(x-xc));
    
    % 插值多项式 (这里只用到两个基函数)
    f = a0*varphi0 + a1*varphi1 + a2*varphi2;
    
    % 构造积分条件
    eqa_mm = int(f, x, xa-2*h, xb-2*h)/h == uavem;
    eqa_m = int(f, x, xa-h, xb-h)/h == uavem;
    eqa_0 = int(f, x, xa,   xb)/h == uave;
    % eqa_p = int(f, x, xa+h, xb+h)/h == uavep; % 目前没用
    
    % 解方程
    s = solve([eqa_0, eqa_m, eqa_mm], [a0, a1, a2]);
    
    % 最终插值函数
    f_hat = simplify(expand(s.a0*varphi0 + s.a1*varphi1 + s.a2*varphi2));
